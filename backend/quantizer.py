...
def quantize_notes(
    ...
    grid_map = {
        'none':     16,   # 1/16 beat ≈ très fine (quasi brut)
    ...
    snap_map = {
        ...
        'light':    4,   # aimantation vers double-croche (snap_step=0.25)
        'standard': 2,   # aimantation vers croche        (snap_step=0.5)
        'heavy':    1,   # aimantation vers noire          (snap_step=1.0)
    }
    ...
    if enable_rubato:
        grid_div = max(grid_div, 32)  # au moins 1/32 beat pour capter le rubato
        if snap_div > 0:
            snap_threshold_ratio *= 0.5  # aimantation plus permissive, mais pas nulle
